...
if [ $? -ne 0 ]; then
   ...
   echo "zmzimletctl -l undeploy com_zextras_drive_open"
   echo "Install the Zimbra Open Drive zimlet."
   echo "zmzimletctl -l deploy /opt/zimbra/zimlets/com_zextras_drive_open.zip"
else
   su - zimbra -c  "zmzimletctl  -l listZimlets" | grep -e "com_zextras_drive_open"
   if [ "$?" -eq "0" ]; then
      echo "removing the opendrive zimlet.."
      su - zimbra -c  "zmzimletctl -l undeploy com_zextras_drive_open"
   fi
   echo "Deploying Zimbra-Opendrive zimlet"
   su - zimbra -c  "zmzimletctl -l deploy /opt/zimbra/zimlets/com_zextras_drive_open.zip"
